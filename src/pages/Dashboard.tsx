  ...
  return (
    <div className={cn(
      ...
      <span className={cn(
        ...
        isCompleted ? "bg-success" : "bg-emerald-500"
      )}></span>
      <span>{status}</span>
    </div>
  );
...
const Tag = ({ children, icon }: { children: React.ReactNode, icon?: React.ReactNode }) => (
  <div className="inline-flex items-center gap-1.5 rounded-md bg-muted/50 px-2 py-1 text-xs font-medium text-muted-foreground border">
    {icon}
    {children}
  </div>
);
...
const Dashboard = () => {
  return (
    <div className="page-transition">
      <section className="w-full max-w-7xl mx-auto px-6 lg:px-8 py-8 space-y-8 bg-gradient-to-br from-background via-background to-muted/20 min-h-screen">
        {/* Header */}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 items-start">
          ...
          >
            <div className="flex justify-end mb-4">
              <Button variant="ghost" size="sm" className="text-primary font-medium hover:bg-primary/10 interactive-button">
                View all
            ...
            <div className="space-y-3">
              {projects.map((project, index) => (
                <div 
                  key={project.id} 
                  className="group rounded-xl border bg-card p-4 transition-all duration-300 hover:shadow-md hover:bg-surface-elevated cursor-pointer animate-slide-in-from-left"
          ...
          >
            <div className="grid grid-cols-1 gap-3">
              {quickActions.map((action, index) => (
                <div
                  key={action.label}
                  className="group flex items-center gap-4 p-4 border rounded-lg bg-card hover:bg-surface-elevated transition-all duration-200 cursor-pointer animate-slide-in-from-right"
